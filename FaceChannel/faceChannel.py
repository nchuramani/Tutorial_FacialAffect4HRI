...
class FaceChannelV1:
    ...
    def __init__(self, type="Cat", loadModel=True, numberClasses=7):
        ...
        if loadModel:
            ...
            if type =="Cat":
                ...
                model = self.getDimensionalModel()
    ...
    def predict(self, images, preprocess=True):
        ...
        if preprocess:
            processedImages = []
            for img in images:
                facePoints, face = facedetect.detectFace(img)
                if not len(face) == 0 and not facePoints == None:  # If a face is detected
                    face = preprocessing.preprocess(face, imageSize=self.IMAGE_SIZE)  # pre-process the face
